...
export async function removeTeamMember(req:Request,res:Response,next:NextFunction){try{const actor=await prisma.usuario.findUniqueOrThrow({where:{id:req.userId!}});if(!actor.isSuperAdmin)throw new Error("Solo el administrador global puede eliminar perfiles");const id=String(req.params.id);if(id===actor.id)throw new Error("No puedes eliminar tu propia cuenta administradora");const target=await prisma.usuario.findUniqueOrThrow({where:{id}});if(target.isSuperAdmin)throw new Error("No se puede eliminar otra cuenta administradora");await prisma.$transaction(async tx=>{await tx.iniciativa.updateMany({where:{responsableId:id},data:{responsableId:null}});await tx.evento.updateMany({where:{asignadoId:id},data:{asignadoId:null}});await tx.evento.deleteMany({where:{creadorId:id}});await tx.progreso.deleteMany({where:{usuarioId:id}});await tx.tareaComentario.deleteMany({where:{usuarioId:id}});await tx.solicitudCambio.updateMany({where:{aprobadorId:id},data:{aprobadorId:null}});await tx.solicitudCambio.deleteMany({where:{solicitanteId:id}});await tx.usuario.delete({where:{id}})});res.json({ok:true})}catch(e){next(isForeignKeyError(e)?Object.assign(new Error("Este perfil tiene actividad (mensajes, notificaciones, tickets, auditoría…) y no se puede eliminar."),{status:409}):e)}}
// La base bloquea el borrado si el perfil aún tiene filas relacionadas (16 tablas
// que removeTeamMember no limpia a propósito: mensajes, auditoría, tickets…).
// Se traduce a un 409 legible en vez de exponer el nombre interno de la restricción.
function isForeignKeyError(error:unknown){return (error as {code?:string})?.code==="P2003"||/Foreign key constraint/i.test(String((error as Error)?.message??""))}
