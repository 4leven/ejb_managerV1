...
import { chatTones, playChatTone, composeSuggestion, suggestedReplies, correctLastWord, readChatAttachment } from "../utils/chat-tools";
...
export default function Messages({
  ...
  const [contacts, setContacts] = useState<Conversation[]>([]),
    ...
    [chatPreferences, setChatPreferences] = useState(() => {
      const defaults={desktop:true,reactions:true,sound:"Tonos",email:false,callSound:true,smartCompose:true,smartReply:true,autocorrect:true,markdown:true,color:"Zafiro",mode:"Sistema",density:"Cómoda",animations:"system",timezone:true};
    ...
    >
      ...
      <aside className="message-space-column">
        ...
        {wallpaperOpen && createPortal(
          <div className={`wallpaper-popover wallpaper-gallery chat-mode-${String(chatPreferences.mode).toLowerCase()} chat-color-${String(chatPreferences.color).toLowerCase()}`}>
            <div>
      ...
      <div className={`messenger ${threadView ? "thread-view" : ""}`}>
        ...
        <section className={`chat-panel ${announcementsOpen ? `announcements-panel ${announcementComposerOpen ? "compose-open" : ""}` : ""}`}>
          {announcementsOpen ? (
            ...
            <>
              ...
              {chatPreferences.smartReply&&messages.length>0&&messages[messages.length-1]?.remitenteId!==currentUserId&&<div className="smart-reply-options" aria-label="Respuestas sugeridas">{suggestedReplies(messages[messages.length-1]?.contenido ?? "").map((reply)=><button type="button" key={reply} onClick={()=>{setDraft(reply);requestAnimationFrame(()=>messageInput.current?.focus())}}>{reply}</button>)}</div>}
              {chatPreferences.smartCompose && composeSuggestion(draft) && <div className="smart-reply-options"><button type="button" onClick={()=>{setDraft(composeSuggestion(draft));messageInput.current?.focus()}}>{composeSuggestion(draft)} · Tab</button></div>}
              ...
              <form onSubmit={submit}>
                ...
                <input
                  ...
                  onKeyDown={(event) => { const suggestion = chatPreferences.smartCompose && composeSuggestion(draft); if (event.key === "Tab" && suggestion) { event.preventDefault(); setDraft(suggestion); } }}
                />
        ...
        {settingsOpen&&createPortal(<div className={`overlay chat-settings-overlay chat-mode-${String(chatPreferences.mode).toLowerCase()} chat-color-${String(chatPreferences.color).toLowerCase()}`} onMouseDown={(event)=>{if(event.target===event.currentTarget)setSettingsOpen(false)}}><section className={`chat-settings-modal density-${String(chatPreferences.density).toLowerCase()}`}><header><h2>Configuración de chat</h2><span className="settings-save-status" role="status">{settingsFeedback}</span><button type="button" aria-label="Cerrar configuración" onClick={()=>setSettingsOpen(false)}><X/></button></header><div className="chat-settings-body"><nav><button type="button" className={settingsTab==="notifications"?"active":""} onClick={()=>setSettingsTab("notifications")}><Bell/>Notificaciones</button><button type="button" className={settingsTab==="messages"?"active":""} onClick={()=>setSettingsTab("messages")}><Inbox/>Mensajes y contenido multimedia</button><button type="button" className={settingsTab==="appearance"?"active":""} onClick={()=>setSettingsTab("appearance")}><Palette/>Aspecto</button><button type="button" className={settingsTab==="accessibility"?"active":""} onClick={()=>setSettingsTab("accessibility")}><Accessibility/>Accesibilidad</button></nav><main className={`chat-settings-content settings-tab-${settingsTab}`}>
          ...
          {settingsTab==="messages"&&<section><h3>Funciones de mensajes</h3><SettingSwitch label="Redacción inteligente" detail="Mostrar sugerencias de escritura proactivas" checked={chatPreferences.smartCompose} onChange={(value)=>updatePreference("smartCompose",value)}/><SettingSwitch label="Respuesta inteligente" detail="Mostrar respuestas sugeridas cuando estén disponibles" checked={chatPreferences.smartReply} onChange={(value)=>updatePreference("smartReply",value)}/><SettingSwitch label="Corrección automática" detail="Corregir errores frecuentes al terminar una palabra y activar el corrector del navegador" checked={chatPreferences.autocorrect} onChange={(value)=>updatePreference("autocorrect",value)}/><SettingSwitch label="Markdown dinámico" detail="Aplicar formato mientras escribes" checked={chatPreferences.markdown} onChange={(value)=>updatePreference("markdown",value)}/></section>}
        ...
        </main></div><footer className="chat-settings-footer"><button type="button" onClick={()=>{setChatPreferences({desktop:true,reactions:true,sound:"Tonos",email:false,callSound:true,smartCompose:true,smartReply:true,autocorrect:true,markdown:true,color:"Zafiro",mode:"Sistema",density:"Cómoda",animations:"system",timezone:true});setSettingsFeedback("Configuración predeterminada restaurada")}}>Restaurar valores</button><span>Las preferencias se guardan automáticamente en este dispositivo.</span><button type="button" className="primary" onClick={()=>setSettingsOpen(false)}><Check/>Guardar y cerrar</button></footer></section></div>,document.body)}
