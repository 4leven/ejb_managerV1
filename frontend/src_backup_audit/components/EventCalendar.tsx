...
export function CalendarModule({ areas, team, items, currentUserId, canManageAll = false, onProjectsChanged }: { areas: any[]; team: any[]; items: any[]; currentUserId: string; canManageAll?: boolean; onProjectsChanged?:()=>void }) {
  ...
  return <>
    <div className="calendar-toolbar"><button className="primary" onClick={() => {setFormError("");setModal(true)}}><Plus />Evento nuevo</button><div><button onClick={() => setAnchor(new Date())}>Hoy</button><button onClick={() => setAnchor(new Date(anchor.getTime() - 604800000))}><ChevronLeft /></button><button onClick={() => setAnchor(new Date(anchor.getTime() + 604800000))}><ChevronRight /></button><b>{days[0].toLocaleDateString()} — {days[6].toLocaleDateString()}</b></div></div>
    ...
    {modal && <div className="overlay"><form className="event-modal" onSubmit={submit}>
      <button type="button" className="close" onClick={() => setModal(false)}><X /></button><CalendarPlus /><h2>Nuevo evento</h2>
